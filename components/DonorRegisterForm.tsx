...
export default function DonorRegisterForm({ presetType }: { presetType?: 'BLOOD' | 'KIDNEY' | 'EYE' }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
  if (donorType === 'BLOOD' || donorType === 'EYE' && (!frontFile || !backFile)) { setError('Please choose both front and back of NIC / License'); return; }
    setSubmitting(true);
    try {
  ...
  if (rh) form.append('rh', rh);
      ...
      const res = await fetch(`${BACKEND_URL}/api/donors/register`, { method: 'POST', body: form });
      if (!res.ok) {
  ...
  <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">Create an account and upload your NIC or Driving License to verify your identity.</p>
  ...
  <div className="flex flex-col gap-3">
          ...
          <input id="email" type="email" required value={email} onChange={e=>setEmail(e.target.value)} className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm dark:border-white/10 dark:bg-slate-800" />
  </div>
  <div>
          <label className="block text-sm font-medium mb-1" htmlFor="password">Password</label>
          <input id="password" type="password" required value={password} onChange={e=>setPassword(e.target.value)} className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm dark:border-white/10 dark:bg-slate-800" />
  </div>
