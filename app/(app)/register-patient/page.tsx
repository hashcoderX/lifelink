...
import { createPatient, PatientPayload } from '@/lib/backend';
...
export default function RegisterPatientPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const created: any = await createPatient(token, payload);
      setMessage(`Patient created with id ${created.id}${created.patient_code ? ' • Code ' + created.patient_code : ''}`);
  setForm({ full_name: '' });
      setHla('');
    } catch (err: any) {
  ...
  return (
    <div className="max-w-5xl mx-auto py-12 px-6">
      <h1 className="text-3xl font-bold mb-4">Register Patient (Backend API)</h1>
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">Temporary form to send patient profile to Laravel API.</p>
      {!token && (
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="email" value={form.new_user_email ?? ''} onChange={e=>update('new_user_email', e.target.value)} className="w-full rounded border px-3 py-2 bg-white dark:bg-neutral-800" placeholder="patient@example.com" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Password</label>
            <input type="password" value={form.new_user_password ?? ''} onChange={e=>update('new_user_password', e.target.value)} className="w-full rounded border px-3 py-2 bg-white dark:bg-neutral-800" placeholder="Min 8 characters" autoComplete="new-password" />
            <p className="text-[11px] text-gray-600 mt-1">If email and password are provided, a patient login will be created.</p>
          </div>
        </div>
